...
impl Detector for AccuracyRiskDetector {
    fn name(&self) -> &'static str { "accuracy_risk" }
    fn description(&self) -> &'static str { "Detects generated code without tests or verification" }
    async fn detect(&self, agent_id: &str, obs: &[serde_json::Value]) -> Vec<DetectedIssue> {
        let has_write = obs.iter().any(|o| o.get("tool").and_then(|v| v.as_str()) == Some("write"));
        ...
        if has_write && !has_test {
            vec![DetectedIssue { id: Uuid::new_v4(), agent_id: agent_id.into(),
                ...
                category: IssueCategory::AccuracyRisk { risk_factors: vec!["no_tests".into()] },
