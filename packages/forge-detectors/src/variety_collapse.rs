...
pub struct VarietyCollapseDetector {
    similarity_threshold: f64,
}

impl VarietyCollapseDetector {
...
impl Detector for VarietyCollapseDetector {
    ...
    fn description(&self) -> &'static str {
        "Detects when agents produce near-identical outputs"
    }
    async fn detect(&self, agent_id: &str, obs: &[serde_json::Value]) -> Vec<DetectedIssue> {
        let outputs: Vec<&str> = obs
            .iter()
            .filter_map(|o| o.get("agent_output").and_then(|v| v.as_str()))
            .collect();
        if outputs.len() < 3 {
            return vec![];
        }
        ...
        for i in 0..outputs.len() {
            for j in i + 1..outputs.len() {
                if outputs[i].len().min(outputs[j].len()) > 0
                    && outputs[i][..200.min(outputs[i].len())]
                        == outputs[j][..200.min(outputs[j].len())]
                {
                    similar += 1;
                }
            }
        ...
        if score > self.similarity_threshold {
            vec![DetectedIssue {
                ...
                category: IssueCategory::VarietyCollapse {
                    similarity_score: score,
                    agent_count: outputs.len() as u32,
                },
                description: format!(
                    "{} agents near-identical ({:.0}%)",
                    outputs.len(),
                    score * 100.0
                ),
                confidence: score,
                suggested_actions: vec!["diversify".into()],
