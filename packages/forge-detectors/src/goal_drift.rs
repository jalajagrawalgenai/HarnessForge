...
impl Detector for GoalDriftDetector {
    ...
    fn description(&self) -> &'static str {
        "Detects when agent diverges from original task"
    }
    async fn detect(&self, agent_id: &str, obs: &[serde_json::Value]) -> Vec<DetectedIssue> {
        let original = obs
            .first()
            .and_then(|o| o.get("original_task"))
            .and_then(|v| v.as_str());
        let current = obs
            .last()
            .and_then(|o| o.get("current_output"))
            .and_then(|v| v.as_str());
        if let (Some(orig), Some(curr)) = (original, current) {
            let sim = if orig.len().min(curr.len()) > 0
                && orig[..50.min(orig.len())] == curr[..50.min(curr.len())]
            {
                1.0
            } else {
                0.3
            ...
            if sim < self.similarity_threshold {
                vec![DetectedIssue {
                    ...
                    category: IssueCategory::GoalDrift {
                        similarity_to_original: sim,
                    },
                    description: format!(
                        "Agent drifted from goal (similarity: {:.0}%)",
                        sim * 100.0
                    ),
                    confidence: 1.0 - sim,
