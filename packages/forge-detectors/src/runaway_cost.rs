...
pub struct RunawayCostDetector {
    acceleration_threshold: f64,
}
impl RunawayCostDetector {
    pub fn new(t: f64) -> Self {
        Self {
            acceleration_threshold: t,
        }
    }
}
...
impl Detector for RunawayCostDetector {
    ...
    fn description(&self) -> &'static str {
        "Detects accelerating cost (2nd derivative)"
    }
    async fn detect(&self, agent_id: &str, obs: &[serde_json::Value]) -> Vec<DetectedIssue> {
        let costs: Vec<f64> = obs
            .iter()
            .filter_map(|o| o.get("cost_per_turn").and_then(|v| v.as_f64()))
            .collect();
        if costs.len() < 6 {
            return vec![];
        }
        ...
        if avg_accel > self.acceleration_threshold {
            vec![DetectedIssue {
                ...
                severity: if avg_accel > self.acceleration_threshold * 2.0 {
                    Severity::Error
                } else {
                    Severity::Warning
                },
                category: IssueCategory::RunawayCost {
