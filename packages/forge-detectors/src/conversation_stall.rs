...
pub struct ConversationStallDetector {
    timeout_secs: u64,
}
impl ConversationStallDetector {
...
impl Detector for ConversationStallDetector {
    ...
    fn description(&self) -> &'static str {
        "Detects when multi-agent conversation has stopped"
    }
        ...
        if let Some(ts) = last_msg {
            ...
            if elapsed >= self.timeout_secs {
                vec![DetectedIssue {
                    ...
                    severity: if elapsed > self.timeout_secs * 2 {
                        Severity::Error
                    } else {
                        Severity::Warning
                    },
                    category: IssueCategory::ConversationStall {
