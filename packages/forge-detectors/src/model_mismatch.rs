...
impl Detector for ModelMismatchDetector {
    ...
    fn description(&self) -> &'static str {
        "Detects wrong model for task complexity"
    }
    async fn detect(&self, agent_id: &str, obs: &[serde_json::Value]) -> Vec<DetectedIssue> {
        for o in obs {
            if let (Some(task), Some(model)) = (
                o.get("task").and_then(|v| v.as_str()),
                o.get("model").and_then(|v| v.as_str()),
            ) {
                let is_complex = COMPLEX_KEYWORDS
                    .iter()
                    .any(|k| task.to_lowercase().contains(&k.to_lowercase()));
                let is_cheap_model =
                    model.contains("haiku") || model.contains("flash") || model.contains("mini");
                if is_complex && is_cheap_model {
                    return vec![DetectedIssue {
                        id: Uuid::new_v4(),
                        ...
                        description: format!(
                            "Complex task '{}' assigned to cheap model {}",
                            task, model
                        ),
                        confidence: 0.85,
                        suggested_actions: vec!["escalate".into()],
                        evidence_summary: format!(
                            "Task complexity mismatch: {} on {}",
                            task, model
                        ),
                    }];
                }
