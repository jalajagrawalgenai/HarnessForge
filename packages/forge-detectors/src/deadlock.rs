...
impl Detector for DeadlockDetector {
    ...
    fn description(&self) -> &'static str {
        "Detects circular wait between agents"
    }
                ...
                if let Some((other_for, _)) = waiting.get(waiting_for) {
                    if other_for == agent {
                        issues.push(DetectedIssue {
                            ...
                            category: IssueCategory::Deadlock {
                                agents: vec![agent.clone(), waiting_for.clone()],
                                wait_duration_secs: *dur,
                            },
                            description: format!("Deadlock: {}↔{} ({}s)", agent, waiting_for, dur),
                            confidence: 1.0,
                            suggested_actions: vec!["reroute".into()],
