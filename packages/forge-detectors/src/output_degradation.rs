...
pub struct OutputDegradationDetector { decline_threshold: f64 }
impl OutputDegradationDetector { pub fn new(t: f64) -> Self { Self { decline_threshold: t } } }

#[async_trait]
impl Detector for OutputDegradationDetector {
    fn name(&self) -> &'static str { "output_degradation" }
    fn description(&self) -> &'static str { "Detects declining output quality over time" }
    async fn detect(&self, agent_id: &str, obs: &[serde_json::Value]) -> Vec<DetectedIssue> {
        let scores: Vec<f64> = obs.iter().filter_map(|o| o.get("quality_score").and_then(|v| v.as_f64())).collect();
        if scores.len() < 4 { return vec![]; }
        ...
        if avg_decline > self.decline_threshold {
            vec![DetectedIssue { id: Uuid::new_v4(), agent_id: agent_id.into(),
                severity: if avg_decline > self.decline_threshold * 2.0 { Severity::Error } else { Severity::Warning },
                category: IssueCategory::OutputDegradation { trend_slope: -avg_decline, consecutive_declines: 3 },
