...
impl Strategy for RollbackStrategy {
    ...
    fn priority(&self) -> u32 { 28 }
    async fn evaluate(&self, detection: &DetectedIssue) -> Option<StrategyResult> {
        Some(StrategyResult { strategy_name: "rollback".into(),
            intervention: Intervention::Rollback { checkpoint_id: Uuid::new_v4(), reason: detection.description.clone() },
