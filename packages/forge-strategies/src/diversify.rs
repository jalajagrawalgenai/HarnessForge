...
impl Strategy for DiversifyStrategy {
    ...
    async fn evaluate(&self, detection: &DetectedIssue) -> Option<StrategyResult> {
        if !matches!(&detection.category, IssueCategory::VarietyCollapse { .. }) {
            return None;
        }
        let intervention = Intervention::Diversify {
            alternative_approach:
                "Try a completely different approach. MVP-first → Risk-first → User-first".into(),
        };
        Some(StrategyResult {
            strategy_name: "diversify".into(),
            intervention,
