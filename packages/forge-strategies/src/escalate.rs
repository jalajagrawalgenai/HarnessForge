...
impl Strategy for EscalateStrategy {
    ...
    async fn evaluate(&self, detection: &DetectedIssue) -> Option<StrategyResult> {
        let new_model = match &detection.category {
            IssueCategory::ModelMismatch { suggested_model, .. } => Some(suggested_model.clone()),
            _ => Some("claude-sonnet-4-6".into()),
        };
        let intervention = Intervention::Escalate { new_model, budget_increase: Some(50000), reason: detection.description.clone() };
        Some(StrategyResult { strategy_name: "escalate".into(), intervention, priority: self.priority(),
