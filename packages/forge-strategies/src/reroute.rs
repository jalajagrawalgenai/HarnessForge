...
impl Strategy for RerouteStrategy {
    fn name(&self) -> &'static str { "reroute" }
    fn priority(&self) -> u32 { 22 }
    async fn evaluate(&self, detection: &DetectedIssue) -> Option<StrategyResult> {
        Some(StrategyResult { strategy_name: "reroute".into(),
            intervention: Intervention::Reroute { to_node: "verify".into(), reason: detection.description.clone() },
