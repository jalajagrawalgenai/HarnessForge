...
impl Observer for AccuracyWatcher {
    fn name(&self) -> &'static str { "accuracy" }
    fn dimension(&self) -> &'static str { "accuracy" }
    async fn observe(&self, event: &AgentEvent) -> Option<serde_json::Value> {
        match event {
            AgentEvent::ToolCallEnd { result, .. } => {
                ...
                Some(serde_json::json!({"dimension":"accuracy","test_pass_rate":rate,"lint_errors":*self.lint_errors.lock().unwrap()}))
            }
            _ => None,
        }
    }
