...
impl SessionReplay {
    ...
    pub fn current(&self) -> Option<&AuditEvent> {
        self.events.get(self.position)
    }
    #[allow(clippy::should_implement_trait)]
    pub fn next(&mut self) -> Option<&AuditEvent> {
        self.position += 1;
        self.current()
    }
    pub fn prev(&mut self) -> Option<&AuditEvent> {
        if self.position > 0 {
            self.position -= 1;
        }
        self.current()
    }
    pub fn seek(&mut self, pos: usize) -> Option<&AuditEvent> {
        self.position = pos.min(self.events.len().saturating_sub(1));
        self.current()
    }
    pub fn jump_to_detection(&mut self, detector: &str) -> Option<&AuditEvent> {
        for (i, e) in self.events.iter().enumerate().skip(self.position) {
            if e.event_type == detector {
                self.position = i;
                return Some(e);
            }
        }
