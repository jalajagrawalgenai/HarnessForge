...
pub struct AlertRule {
    pub detector: String, pub min_severity: Severity,
    pub channels: Vec<String>, pub throttle_secs: u64,
}
...
impl AlertEngine {
    ...
    pub fn evaluate(&self, issue: &DetectedIssue) -> Vec<String> {
        self.rules.iter().filter(|r| r.detector == issue.category_name() && issue.severity >= r.min_severity)
            .flat_map(|r| r.channels.clone()).collect()
    }
}
