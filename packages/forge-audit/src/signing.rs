...
pub fn compute_hash_chain(events: &[AuditEvent], genesis: &str) -> Vec<String> {
    ...
    for event in events {
        ...
        hasher.update(serde_json::to_string(&event.event_data).unwrap_or_default().as_bytes());
...
pub fn verify_integrity(events: &[AuditEvent], genesis: &str) -> bool {
    ...
    for event in events {
        ...
        h.update(serde_json::to_string(&event.event_data).unwrap_or_default().as_bytes());
        let computed = hex::encode(h.finalize());
        if computed != event.hash_chain { return false; }
        expected = computed;
    }
    true
}
