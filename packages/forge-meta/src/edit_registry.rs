...
impl EditRegistry {
    ...
    pub fn current_version(&self) -> String {
        self.versions.last().map(|v| v.version.clone()).unwrap_or_else(|| "v1.0.0".into())
    }
    ...
    pub fn rollback(&mut self) -> Result<String, ForgeError> {
        if self.versions.len() <= 1 {
            return Err(ForgeError::Internal("No previous version to rollback to".into()));
        }
        // Remove latest
        self.versions.pop();
    ...
    pub fn versions(&self) -> &[HarnessVersion] { &self.versions }
    pub fn edits(&self) -> &[HarnessEdit] { &self.applied_edits }
    pub fn total_edits(&self) -> usize { self.applied_edits.len() }
    ...
    pub fn diff(&self, v1: &str, v2: &str) -> Option<Vec<&HarnessEdit>> {
        ...
        Some(self.applied_edits[start..end.min(self.applied_edits.len())].iter().collect())
    }
}
