...
fn make_audits(count: usize, success_rate: f64) -> Vec<SessionAudit> {
    (0..count).map(|i| SessionAudit {
        session_id: format!("session_{}", i),
        agent_type: "solo".into(), model: if i % 3 == 0 { "claude-sonnet".into() } else { "claude-haiku".into() },
        success: (i as f64) < (count as f64 * success_rate),
        total_tokens: 5000 + (i * 100) as u64, total_cost: 0.02 + (i as f64 * 0.001),
        duration_secs: 60.0 + (i as f64 * 2.0),
        detection_count: if i % 5 == 0 { 2 } else { 1 },
        intervention_count: if i % 5 == 0 { 2 } else { 1 },
        failure_patterns: if i % 3 == 0 { vec!["loop_detected".into()] } else { vec![] },
        error_messages: if i % 4 == 0 { vec!["context overflow".into()] } else { vec![] },
    }).collect()
}
...
fn test_weakness_miner_finds_patterns() {
    ...
    assert!(!patterns.is_empty(), "Should find at least one pattern with 60 sessions");
    // First pattern should have highest severity
    if patterns.len() > 1 {
        assert!(patterns[0].severity_score >= patterns[1].severity_score);
    }
    assert!(patterns.iter().any(|p| p.model_family != "all" || p.occurrence_count >= 3));
}
...
fn test_edit_registry_versioning() {
    ...
    let edit = forge_meta::harness_proposer::HarnessEdit {
        id: "edit_1".into(), weakness_id: "pat_1".into(),
        target: forge_meta::harness_proposer::EditTarget::DetectorThreshold { detector: "loop".into(), parameter: "threshold".into() },
        change: forge_meta::harness_proposer::EditChange::ModifyValue { old: serde_json::json!(6), new: serde_json::json!(4) },
        rationale: "Lower threshold".into(), affected_models: vec!["claude".into()], min_chars: 30,
    };
...
fn test_edit_registry_rollback() {
    let mut registry = EditRegistry::new();
    let edit = forge_meta::harness_proposer::HarnessEdit {
        id: "edit_x".into(), weakness_id: "pat_x".into(),
        target: forge_meta::harness_proposer::EditTarget::PresetUpdate { preset: "solo".into() },
        change: forge_meta::harness_proposer::EditChange::ModifyValue { old: serde_json::json!("a"), new: serde_json::json!("b") },
        rationale: "test".into(), affected_models: vec![], min_chars: 10,
    };
    registry.apply(edit).unwrap();
...
fn test_scheduler_triggers() {
    ...
    for _ in 0..10 { scheduler.notify_session_completed(); }
    assert!(scheduler.should_run());
    scheduler.mark_ran();
    assert!(!scheduler.should_run());
}
...
fn test_scheduler_daily_limit() {
    ...
    // Run max times
    for i in 0..4 {
        for _ in 0..3 { scheduler.notify_session_completed(); }
        assert!(scheduler.should_run(), "Run {} should be allowed", i + 1);
        scheduler.mark_ran();
    }
    for _ in 0..3 { scheduler.notify_session_completed(); }
    assert!(!scheduler.should_run(), "Should hit daily limit after 4 runs");
}

#[test]
fn test_meta_harness_insufficient_sessions() {
    let config = MetaConfig { min_sessions: 100, ..MetaConfig::default() };
    let mut meta = MetaHarness::new(config);
    let audits = make_audits(10, 0.5);
    let rt = tokio::runtime::Runtime::new().unwrap();
    let result = rt.block_on(meta.improve(&audits, &serde_json::json!({}), &[])).unwrap();
...
fn test_session_audit_serialization() {
    let audit = SessionAudit {
        session_id: "s1".into(), agent_type: "solo".into(), model: "claude".into(),
        success: true, total_tokens: 1000, total_cost: 0.01, duration_secs: 30.0,
        detection_count: 1, intervention_count: 1,
        failure_patterns: vec!["loop".into()], error_messages: vec![],
    };
