...
pub struct PagerDutyIntegration { routing_key: String }
impl PagerDutyIntegration {
    pub fn new(key: &str) -> Self { Self { routing_key: key.into() } }
    ...
    pub fn resolve(&self, dedup: &str) -> String {
        json!({"routing_key":self.routing_key,"event_action":"resolve","dedup_key":dedup}).to_string()
    }
}
