...
static MCP_CLIENT: std::sync::LazyLock<Mutex<McpClient>> = std::sync::LazyLock::new(|| Mutex::new(McpClient::new()));

pub async fn list_servers() -> Json<Value> {
    let client = MCP_CLIENT.lock().unwrap();
    let servers: Vec<Value> = client.servers().iter().map(|s| json!({"name":s.name,"transport":s.transport,"endpoint":s.endpoint})).collect();
...
pub async fn add_server(Json(body): Json<Value>) -> Json<Value> {
    ...
    let transport = body.get("transport").and_then(|v|v.as_str()).unwrap_or("stdio");
    let endpoint = body.get("endpoint").and_then(|v|v.as_str()).unwrap_or("");
    let mut client = MCP_CLIENT.lock().unwrap();
    client.add_server(McpServerConfig{name:name.into(),transport:transport.into(),endpoint:endpoint.into()});
...
pub async fn discover() -> Json<Value> {
    let servers: Vec<Value> = McpDiscovery::scan().iter().map(|s| json!({"name":s.name,"transport":s.transport,"endpoint":s.endpoint})).collect();
    Json(json!({"discovered":servers,"count":servers.len()}))
}
pub async fn gateway_status() -> Json<Value> { Json(json!({"gateway":"not_started","port":0,"upstream_count":0})) }
...
pub async fn forge_server_status() -> Json<Value> { Json(json!({"mcp_server":"not_started","port":0})) }
