...
pub struct SearchQuery { pub q: Option<String>, pub plugin_type: Option<String> }

pub async fn search(Query(q): Query<SearchQuery>) -> Json<Value> {
    let query = q.q.as_deref().unwrap_or("");
    Json(json!({"plugins":[],"query":query,"total":0,"message":"Marketplace registry not connected"}))
}
