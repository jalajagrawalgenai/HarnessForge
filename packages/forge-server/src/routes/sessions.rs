...
pub async fn stream(Path(id): Path<String>) -> Json<Value> {
    Json(json!({"stream":format!("sse://sessions/{}",id)}))
}
pub async fn checkpoints(Path(_id): Path<String>) -> Json<Value> {
    Json(json!({"checkpoints":[]}))
}
pub async fn pause(Path(id): Path<String>) -> Json<Value> {
    Json(json!({"paused":id}))
}
pub async fn resume(Path(id): Path<String>) -> Json<Value> {
    Json(json!({"resumed":id}))
}
