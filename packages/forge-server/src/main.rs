...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/v1/sessions", get(routes::sessions::list).post(routes::sessions::create))
        .route("/v1/sessions/:id", get(routes::sessions::get).delete(routes::sessions::delete))
        .route("/v1/sessions/:id/stream", get(routes::sessions::stream))
        .route("/v1/sessions/:id/audit", get(routes::audit::get_audit))
        .route("/v1/sessions/:id/audit/report", get(routes::audit::get_report))
        .route("/v1/sessions/:id/audit/export", get(routes::audit::export_audit))
        .route("/v1/sessions/:id/audit/replay", get(routes::audit::replay))
        .route("/v1/sessions/:id/checkpoints", get(routes::sessions::checkpoints))
        .route("/v1/sessions/:id/pause", axum::routing::post(routes::sessions::pause))
        .route("/v1/sessions/:id/resume", axum::routing::post(routes::sessions::resume))
        .route("/v1/harness", get(routes::harness::get).put(routes::harness::update))
        .route("/v1/harness/versions", get(routes::harness::versions))
        .route("/v1/meta/improve", axum::routing::post(routes::meta::improve))
        ...
        .route("/v1/analytics/interventions", get(routes::analytics::interventions))
...
async fn status() -> Json<Value> { Json(json!({"version":"0.1.0","uptime":0})) }
